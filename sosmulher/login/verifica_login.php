...
if ( 
  ...
  ! empty ( $dados_usuario['senha'] ) 
	...
	if ( MD5( $dados_usuario['senha']) === $fetch_usuario['user_password'] ) {
		...
		$_SESSION['user_phone']   		 = $fetch_usuario['user_phone'];
		$_SESSION['user_emergency']   	 = $fetch_usuario['user_emergency'];
		$_SESSION['user_telemergency']   = $fetch_usuario['user_telemergency'];
	} else {
		// Continua deslogado
